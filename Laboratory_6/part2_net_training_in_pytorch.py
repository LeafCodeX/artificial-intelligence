...
def training(model, x, y):
    ...
    for _ in range(n_steps):
        ...
        # (DONE) TODO losowa paczka (mini-batch) danych o rozmiarze minibatch_size
        #  (użyj torch.randint do wylosowania indeksów przykładów
        index = torch.randint(0, x.shape[0], (minibatch_size,)) # generuje losowe indeksy z zakresu danych treningowych, liczba indeksów = minibatch_size
        x_batch = x[index] # wybieram podzbiór danych treningowych
        y_batch = y[index] # wybieram podzbiór etykiet

        # (DONE) TODO forward pass modelu + policzenie wartości funkcji kosztu (użyj loss_fn zdefiniowanego wyżej)
        y_pred = model.forward(x_batch) # przewiduje wartości dla wybranej paczki danych
        loss = loss_fn(y_pred, y_batch) # obliczam wartość funkcji kosztu (używam binary_cross_entropy)
        ...
        # (DONE) TODO update params
        with torch.no_grad(): # gdy nie chcę liczyć gradientów (podczas aktualizacji wag)
            for param in model.parameters():
                param -= learning_rate * param.grad # aktualizacje wagi modelu na podstawie gradientów
