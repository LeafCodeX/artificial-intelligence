...
class Connect4:
    ...
    def draw(self):
        for row in self.board:
            print(' '.join(row))
        if self.game_over:
            print('>> Game over! Wins => [', self.wins, ']')
            print(f"===========================================================================================")
        else:
            print('>> Current move: [', self.who_moves, ']')
            print('>> Possible drops:', self.possible_drops())

    def copy(self):
        game_copy = Connect4(width=self.width, height=self.height)
        game_copy.who_moves = self.who_moves
        game_copy.game_over = self.game_over
        game_copy.wins = self.wins
